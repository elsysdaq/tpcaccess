 ...
 *
 * This example demonstrates an example use case for the TpcAccess C++ API
 * It shows how to build up a TPC_System, how to set recording parameters
 * and how to read out the device status as well as the measurement data.
 *
...
int main() {
    ...
    if (!CheckErrorCode(err)) return 1;
    ...
    */
    ...
    cout << endl
         << "Meas. Start Time: " << tmetaData.startTime.hour << ":" << tmetaData.startTime.minute << ":"
...
void TPC_CC StatusCallback(void* userData, int deviceIx, struct TPC_DeviceStatus* status) {
    ...
    switch (status->boards[0].recordingState) {
        case tpc_recAborted:
            ...
            break;
        case tpc_recStarting:  cout << "Board 0 Recording state: started." << endl; break;
        case tpc_recRecording: cout << "Board 0 Recording state: recording." << endl; break;
        case tpc_recStopped:
            m_recordingStopped = true;
