...
configuring a TransPC system and read out their measurement data.  A TransPC system has a server/client architecture
with the hardware host device that is equipped with TPCE PCIe cards acting as a server and the host on which running the user
application acting as the client. The communication between the server and client is TCP/IP based and may occur via 
physically seperate hosts which are connected over a network.

This API is the interface to the user application and runs on the client side. All parameters and settings
are managed on the server side. Multiple clients may safely connect to one server.

When a DAQ system consists of multiple servers they must be connected to a SyncLink to ensure synchronization.
...
\ref Cluster\n
If there are several TPCE modules (boards) in the system it usefull to group them together (forming clusters)
...
enum TPC_ErrorCode {
    ...
    tpc_errHardwareFailure = 5,

    /// A network or API compatibility error occurred while communicating with the device.
    tpc_errTransmissionError = 6,
...
/// Max number of strain input ranges
#define tpc_maxStrainInputRanges 12
...
struct TPC_DeviceInfo {
    ...
    /// Model Type of the measurement instrument in which the TPCX cards are installed to. 0 = Standard, 1 = TraNET-FE
    /// (read only)
    int ModelType;
...
/**
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param deviceSettings A variable with the new settings. Only read and write classified struct members will be
 * written.
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
...
enum TPC_SyncLinkMode { tpc_sync_oldserver = 0x00, tpc_sync_notdetected = 0x01, tpc_sync_detected = 0x02 };

/// Firmware Capability Information
enum TPC_FirmwareCapabilities {
    /// Indicates support for extended block sizes on DAQ card
    tpc_capExtendedBlockSizes = 0x01,

    /// 0x02 - 0x08 are used internally

    /// Indicates support for preprocessing with digital filters on DAQ card
    tpc_capDigitalFilter = 0x10,
};
...
struct TPC_BoardInfo {
    ...
    /// Firmware capabilities bit field. See \ref TPC_FirmwareCapabilities for the meaning of the bits.
    int firmwareCapabilities;
};
 ...
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetBoardInfo(int deviceIx, int boardAddress, struct TPC_BoardInfo* boardInfo,
                                                   int structSize);
...
struct TPC_InputInfo {
    ...
    /// See \ref TPC_OffsetOptions for the meaning of the bits.
    int offsetOptions;

    /// Filter module options bit field.
    /// See \ref TPC_FilterOptions for the meaning of the bits.
    int filterOptions;

    /// Differential input module options bit field.
    /// See \ref TPC_DifferentialInputOptions for the meaning of the bits.
    int diffOptions;
...
/// Get the list of input range for a specific input type and input
/**
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param boardAddress The address of the board.
 * \param inputNumber The number of the input (0-based).
 * \param inputType Input coupling type \ref TPC_InputCoupling
 * \param inputRanges A variable to receive information about the input ranges
 * \param nrOfRanges Number of Ranges expected
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
...
enum TPC_Parameter {
    ...
    /// Strain Gauge Input Parameter

    /// Strain Excitation voltage
    tpc_parExcVoltage = 80,

    /// Quarter or Half-Bridge
    tpc_parStrainHalfQuarter = 81,

    /// 4 or 6 wire measurement
    tpc_parExcVoltageSense = 82,

    /// start auto zero process
    tpc_parStrainAutoZero = 83,
};
...
enum TPC_TriggerComparatorMode {
    ...
    /// Triggers when state is outside a window or enables other triggers when is used in a AND trigger group, can only
    /// used for trigger comparator 1
    tpc_trgCompStateWindowOut = 6,

    /// Triggers when state is inside a window or enables other triggers when is used in a AND trigger group, can only
    /// used for trigger comparator 1
...
enum TPC_EasyTriggerComperatorMode {
    /// Triggers on positive slope; Triggers when signal goes inside window in Window mode; Triggers when signal above
    /// level in State mode
    tpc_etrgCompPositive = 1,
    /// Triggers on negative slope; Triggers when signal goes outside window in Window mode; Triggers when signal below
    /// level in State mode
...
/**
 * The I2C serial protocol trigger fully supports the Standard-mode, Fast-mode, Fast-mode Plus and Ultra Fast-mode I2C
 * protocol specifications. Only one serial protocol trigger can be specified per board. Refer to \ref
 * TPC_I2CTriggerMode for an overview over all trigger possibilities. The fields starting with "adr" and the "ack" field
 * are only validated in Address or AddressAndData mode.  The fields starting with "data" are only validated in
 * AddressAndData mode.
 */
struct TPC_I2CTriggerSettings {
    ...
    /// Analog level in volts. All analog input values from the data signal below this value are interpreted as
    /// logically 0. This parameter is ignored if chDataInputSignal is not set to Analog.
    double chDataLevelL;
    /// Analog level in volts. All analog input values from the data signal above this value are interpreted as
    /// logically 1. This parameter is ignored if chDataInputSignal is not set to Analog.
    double chDataLevelH;
    ...
    /// Analog level in volts. All analog input values from the clock signal below this value are interpreted as
    /// logically 0. This parameter is ignored if chClockInputSignal is not set to Analog.
    double chClockLevelL;
    /// Analog level in volts. All analog input values from the clock signal above this value are interpreted as
    /// logically 1. This parameter is ignored if chClockInputSignal is not set to Analog.
    ...
    /// Defines wether to trigger on write or request for read messages only or on both message types. This condition is
    /// only evaluated in address or address and data trigger mode.
    TPC_I2CRWBit adrRWBit;
    /// Defines wether to trigger on 7 bit or 11 bit addressing messages. This condition is only evaluated in address or
    /// address and data trigger mode.
    TPC_I2CAdrType adrType;
    /// If true, trigger is generated when adrLow <= address <= adrHigh. If false, trigger is generated when address >
    /// adrHigh or address < adrLow.
    bool adrInRange;
    /// Lower end of the address range.
    int adrLow;
    /// Upper end of the address range.
    int adrHigh;
    /// All bits marked with zero in this mask are ignored for the address range check. If you do not want to make use
    /// of any address masking set this parameter to 0xffff.
    int adrMask;

    /// Specifies from which data byte onward the data should be compared (0 based). The data byte at this position is
    /// compared to the array entry at index 0. Maximum value is 253.
    int dataStartByte;
    /// Specifies the number of data bytes to compare. Valid range: 1 to 16.
    int dataLength;
    /// If true, trigger is generated when dataLow <= data <= dataHigh. If false, trigger is generated when data >
    /// dataHigh or data < dataLow.
    bool dataInRange;
    /// Lower end of the data range.
    char dataLow[16];
    /// Upper end of the data range.
    char dataHigh[16];
    /// All bits marked with zero in this mask are ignored for the data range check. If you do not want to make use of
    /// any data masking set all data bytes in this mask to 0xff.
    char dataMask[16];
};
 ...
 * are only validated in Remote, Id or IdAndData mode.  The fields starting with "data" are only validated in IdAndData
 * mode.
 */
struct TPC_CANTriggerSettings {
    ...
    int bitRate;
    /// The position of the sample point within the nominal bit time. Valid values from 0 (start of bit time) to 1 (end
    /// of bit time). Typically 0.7
    double bitSamplePoint;

    /// Defines on which frame formats should be triggered. This condition is only evaluated in Id or IdAndData trigger
    /// mode.
    TPC_CANIdFormat idFormat;
    /// If true, trigger is generated when idLow <= id <= idHigh. If false, trigger is generated when id > idHigh or id
    /// < idLow.
    bool idInRange;
    ...
    int idHigh;
    /// All bits marked with zero in this mask are ignored for the id range check. If you do not want to make use of any
    /// id masking set this parameter to 0xffffffff.
    int idMask;
...
enum TPC_ParameterAvailableValuesType {
    /// Indicates that server returned a list of discrete values which are available to some parameter
    tpc_valsDiscrete = 0,

    /// Indicates that server returned a list of pairwise defined intervals which are available to some parameter
    tpc_valsIntervals = 1,

    /// Indicates that server returned a list of pairwise defined intervals which are available to some parameter
    /// Additionally, it indicates that only integers may be considered to be valid values for that parameter
    tpc_valsIntegerIntervals = 2,
};

/// Returns a parameter's available values.
/**
 * This function is currently unsupported for most parameter types
 * Supported parameters include tpc_parScopeBlockLength and tpc_parBlkBlockLength
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param boardAddress The address of the board.
 * \param inputNumber The number of the input (0-based).
 * \param parameter The parameter for which the available values should be retrieved.
 * \param type Describes how to interpret the array of available values, see \ref TPC_ParameterAvailableValuesType.
 * \param values Pointer to API-allocated array which holds double values, the user must manually manage this
 *               array's lifetime by calling \ref TPC_FreeParameterAvailableValues once the data may be freed from
 * memory.
 * \param length The length of the "values" array (in number of double values).
 * \returns
 * - \ref tpc_noError if successful.
 * - \ref tpc_errNotImplemented if the server does not support returning available values for this parameter.
 * - another error code indicating a problem with the parameters, the devices or the network connection.
 */
...
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_FreeParameterAvailableValues(double* values);
 ...
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_SetAttribute(int deviceIx, int boardAddress, int inputNumber, const char* key,
                                                   const char* value);
 ...
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetAttribute(int deviceIx, int boardAddress, int inputNumber, const char* key,
 ...
 */
typedef bool(TPC_CC* TPC_AttributeEnumeratorCallback)(void* userData, int boardAddress, int inputNumber,
 ...
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetAllAttributes(int deviceIx, TPC_AttributeEnumeratorCallback callback,
                                                       void* userData);
 ...
 * \returns - tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_SetTrigger(int deviceIx, int boardAddress, int inputNumber,
                                                 TPC_EasyTriggerMode mode, TPC_EasyTriggerComperatorMode compMode,
                                                 TPC_EasyTriggerFlags flags, double level, double hysteresis,
                                                 int time = 0, int time2 = 0);
 ...
 * \returns - tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetTrigger(int deviceIx, int boardAddress, int inputNumber,
                                                 TPC_EasyTriggerMode* mode, TPC_EasyTriggerComperatorMode* compMode,
                                                 TPC_EasyTriggerFlags* flags, double* level, double* hysteresis,
                                                 int* time, int* time2);

/// \brief Resets the serial protocol trigger for the specified board. After this command the serial protocol trigger is
/// no more generating any triggers.
/**
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param boardAddress The address of the board.
 * \returns - tpc_noError if successful, otherwise an error code indicating the problem.
...
/// \brief Returns the current configuration of the I2C protocol trigger for the specified board. Check with \ref
/// TPC_GetSerTrgProtocol beforehand if this function is applicable.
/**
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param boardAddress The address of the board.
 * \param settings The trigger specification settings.
...
/// \brief Returns the current configuration of the CAN trigger for the specified board. Check with \ref
/// TPC_GetSerTrgProtocol beforehand if this function is applicable.
/**
 * \param deviceIx The device index as returned by \ref TPC_AddDevice.
 * \param boardAddress The address of the board.
...
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetYMetaData(int deviceIx, int boardAddress, int inputNumber,
                                                   int measurementNumber, struct TPC_YMetaData* metaData,
                                                   int structSize);
...
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetMetaDataParameter(int deviceIx, int boardAddress, int inputNumber,
                                                           int measurementNumber, enum TPC_Parameter parameter,
                                                           double* value);
 ...
 * \returns - \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetAllMetaDataAttributes(int deviceIx, int measurementNumber,
...
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetTMetaData(int deviceIx, int boardAddress, int blockNumber,
                                                   int measurementNumber, struct TPC_TMetaData* metaData,
                                                   int structSize);
...
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetRawData(int deviceIx, int boardAddress, int inputNumber, int blockNumber,
                                                 int measurementNumber, uint64_t dataStart, int dataLength,
                                                 int32_t* data);
 ...
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_DeferredGetRawData(int deviceIx, int boardAddress, int inputNumber,
                                                         int blockNumber, uint64_t dataStart, int dataLength,
                                                         int32_t* data, enum TPC_ErrorCode* error);
 ...
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_DeferredGetMinMaxData(int deviceIx, int boardAddress, int inputNumber,
 ...
 * \returns \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_DeferredGetRawMinMaxData(int deviceIx, int boardAddress, int inputNumber,
                                                               int blockNumber, uint64_t dataStart, uint64_t dataLength,
                                                               int resultLength, int32_t* data,
                                                               enum TPC_ErrorCode* error);
