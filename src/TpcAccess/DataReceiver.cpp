...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: DataReceiver.cpp 45 2025-03-19 12:46:37Z philipp $
...
void DataRequest::SetData(uint16_t* source, int length) {
    if (m_scale) {
        for (int i = 0; i < length; i++) ((double*)m_dataPtr)[i] = (double)((source[i] & m_mask) * m_f + m_c);
    }
    else {
        for (int i = 0; i < length; i++) ((int32_t*)m_dataPtr)[i] = (uint16_t)source[i];
    }
}
...
void DataRequest::SetData(uint32_t* source, int length) {
    if (m_scale) {
        ...
        for (int i = 0; i < length; i++) ((int32_t*)m_dataPtr)[i] = (int32_t)source[i];
    }
}

DataRangeRequest::DataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                   uint64_t dataLength, int resultLength, void* data, TPC_ErrorCode* error)
...
DataMinMaxRequest::DataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                     uint64_t dataLength, int resultLength, void* data, TPC_ErrorCode* error)
    : DataRequest(boardAddress, inputNumber, blockNumber, data, error) {
    m_dataStart    = dataStart;
    m_dataLength   = dataLength;
...
DataEnvelopeRequest::DataEnvelopeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t* indices,
                                         int numIndices, void* data, TPC_ErrorCode* error)
    : DataRequest(boardAddress, inputNumber, blockNumber, data, error) {
...
DataEnvelopeRequest::DataEnvelopeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t* indices,
                                         int numIndices, void* data, TPC_ErrorCode* error, uint32_t mask,
                                         double voltFactor, double voltConstant)
    : DataRequest(boardAddress, inputNumber, blockNumber, data, error, mask, voltFactor, voltConstant) {
    m_indices = new uint64_t[numIndices];
    for (int i = 0; i < numIndices; i++) m_indices[i] = indices[i];
    m_numIndices = numIndices;
...
void DataReceiver::AddDataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                       uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error,
                                       uint32_t mask, double factor, double constant) {
    DataRangeRequest* r = new DataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength,
                                               resultLength, data, error, mask, factor, constant);
    m_requests.push_back(r);
}

void DataReceiver::AddRawDataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                          uint64_t dataLength, int resultLength, int32_t* data, TPC_ErrorCode* error) {
    DataRangeRequest* r =
        new DataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength, data, error);
    m_requests.push_back(r);
}

void DataReceiver::AddDataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                        uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error,
                                        uint32_t mask, double factor, double constant) {
    DataMinMaxRequest* r = new DataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength,
                                                 resultLength, data, error, mask, factor, constant);
    m_requests.push_back(r);
}

void DataReceiver::AddRawDataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                           uint64_t dataLength, int resultLength, int32_t* data, TPC_ErrorCode* error) {
    DataMinMaxRequest* r =
        new DataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength, data, error);
    m_requests.push_back(r);
}

ns__DataSpecificationArray DataReceiver::PrepareForSoap(struct soap* soap) {
    int n = static_cast<int>(m_requests.size());
        ...
        if (m_hdrToGo > 0) {
            ...
            if (m_hdrToGo == 0) {
                ...
                if (m_dataToGo == 0)
                    ...
                    m_dataBlock = new uint8_t[m_dataToGo];
            ...
            if ((unsigned int)req->DataLength() != m_hdr.numberOfSamples) {
                ...
                if (m_dataBlock != NULL && m_hdr.error == 0 && m_hdr.numberOfSamples > 0) {
                    switch (m_hdr.sampleFormat) {
                        ...
                        default: req->SetError(tpc_errInternalError); break;
                    }
                }
            }
            if (m_dataBlock != NULL) {
