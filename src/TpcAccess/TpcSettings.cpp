 ...
 */
//---------------------------------------------------------------------------
// $Id: TpcSettings.cpp 37 2024-01-29 15:58:48Z roman $
...
const std::vector<Param> kParamTable{
...
int SearchParamName(const nstring& name) {
    for (int i = 0; i < kParamTable.size(); ++i) {
        if (kParamTable[i].name == name) return i;
    }
    return -1;
}
...
class ResourceBasedValidatingDOMInOut : public ValidatingDOMInOut {
   public:
    ResourceBasedValidatingDOMInOut(HINSTANCE hinstance_for_schema, const nstring& schema_resource_name)
        : hinstance(hinstance_for_schema),
...
class ResourceBasedValidatingDOMInOut : public ValidatingDOMInOut {
   public:
    ResourceBasedValidatingDOMInOut(const nstring& schema_resource_name) : schema_name(schema_resource_name) {}
...
TPC_ErrorCode WriteSettingFile(const std::string& filename, std::string& errormsg) {
    ...
    try {
        ...
        if (tcpsettings.WriteFile(settings, StringToNstring(filename).c_str())) {
            return tpc_noError;
        }
        else {
            errormsg = NstringToString(tcpsettings.GetLastError());
...
#endif
    try {
        DOMDocumentHandle doc(tcpsettings.ParseFile(StringToNstring(filename).c_str()));
        if (doc) {
