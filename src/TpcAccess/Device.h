...
--------------------------------------------------------------------------------*/
#ifndef TPCACCESS_DEVICE_H
#define TPCACCESS_DEVICE_H
...
class Device {
   ...
   public:  // Device info
    ...
    TPC_ErrorCode GetInputRanges(int boardAddress, int inputNumber, int inputType, double* inputRanges, int size);
   ...
   public:  // Parameters and Attributes
    ...
    TpcExpect<ParameterAvailableValues> GetParameterAvailableValues(int boardAddress, int inputNumber,
    ...
    TPC_ErrorCode GetAttribute(int boardAddress, int inputNumber, const char* key, std::string& value);
    ...
    TPC_ErrorCode SetTrigger(int boardAddress, int inputNumber, TPC_EasyTriggerMode mode,
                             TPC_EasyTriggerComperatorMode comp, TPC_EasyTriggerFlags flags, double level,
                             double hysteresis, int time, int time2);
    TPC_ErrorCode GetTrigger(int boardAddress, int inputNumber, TPC_EasyTriggerMode* mode,
                             TPC_EasyTriggerComperatorMode* comp, TPC_EasyTriggerFlags* flags, double* level,
                             double* hysteresis, int* time, int* time2);
   ...
   public:  // Data readout
    TPC_ErrorCode GetYMetaData(int boardAddress, int inputNumber, int measurementNumber, TPC_YMetaData* metaData,
                               int structSize);
    ...
    TPC_ErrorCode GetMetaDataAttribute(int boardAddress, int inputNumber, int measurementNumber, const char* key,
                                       std::string& value);
    TPC_ErrorCode GetAllMetaDataAttributes(int measurementNumber, TPC_AttributeEnumeratorCallback callback,
                                           void* userData);
    TPC_ErrorCode GetMetaDataAssociatedChannels(int boardAddress, int inputNumber, int measurementNumber,
                                                TPC_AssociatedChannel* list, int* count);
    TPC_ErrorCode GetTMetaData(int boardAddress, int blockNumber, int measurementNumber, TPC_TMetaData* metaData,
                               int structSize);
    TPC_ErrorCode GetTMetaData(int boardAddress, int blockNumberFrom, int blockNumberTo, int measurementNumber,
                               TPC_TMetaData* metaData, int structSize);

    enum ReadoutType { readoutRange, readoutMinMax };

    TPC_ErrorCode GetData(int boardAddress, int inputNumber, int blockNumber, int measurementNumber, uint64_t dataStart,
                          uint64_t dataLength, int resultLength, double* data, ReadoutType type);
    TPC_ErrorCode GetRawData(int boardAddress, int inputNumber, int blockNumber, int measurementNumber,
                             uint64_t dataStart, uint64_t dataLength, int resultLength, int32_t* data,
                             ReadoutType type);

    TPC_ErrorCode DeferredGetData(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                  uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error,
                                  ReadoutType type);
    TPC_ErrorCode DeferredGetRawData(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
   ...
   private:  // Initialization
    ...
    void addRef();
    [[nodiscard]] bool dropRef();  // Returns true when refcount reached zero, caller must delete/finalize the object.
   ...
   private:  // Device status
    ...
    void SetPreviousParamAttr(const ParameterSet& params, const Attributes& attr, const YMetaDataList& metadata,
   ...
   private:  // Parameters and attributes
    ...
    bool m_availableValuesStaleFlag;
    ...
    // TODO why do these not use m_soap
    ...
    TpcExpect<ParameterAvailableValues> DefaultAvailableValues(int boardAddress, TPC_Parameter parameter);
   ...
   public:
    ...
    // New functionality
    SoapDevice* getSoapConnection() { return m_soap; };
};

//---------------------------------------------------------------------------------
#endif  // TPCACCESS_DEVICE_H
