 ...
 */
//---------------------------------------------------------------------------
// $Id: TpcSettingsToXml.cpp 48 2025-03-27 12:45:02Z philipp $
...
static const char* const Empty_TcpSettings_XML =
    ...
    "<TpcSettings xmlns='http://www.elsys.ch/schema/transas/tpcsettings-1.x.xsd' Version='1.0' >"  // <<<--- TpcSettings
                                                                                                   // Version
    "</TpcSettings>";
...
#else
   public:
    XSTR(const TCHAR* s) { x = X::XMLString::transcode(s); }
    ~XSTR() { X::XMLString::release(&x); }
    operator const XMLCh*() { return x; }
...
static std::string GetDeviceUrl(int index) {
    ...
    for (;;) {
        TPC_ErrorCode status = TPC_GetDeviceUrl(index, buf.GetPtr(), static_cast<int>(buf.GetSize()));
        if (status == tpc_noError) {
            return buf.GetPtr();
        }
...
static bool TPC_CC AttributeEnumeratorCallback(void* userData, int boardAddress, int inputNumber, const char* key,
                                               const char* value) {
    X::DOMElement* e_attributes = reinterpret_cast<X::DOMElement*>(userData);
    std::string value_str(value);
    std::string key_str(key);
    AddAttribute(AddAttribute(AddAttribute(AddLeaf(e_attributes, L"Attribute", StringToNstring(value_str)), L"BoardAddress",
                                           IntToString(boardAddress)),
                              L"InputNumber", IntToString(inputNumber)),
                 L"Name", StringToNstring(key_str));
    return true;
}
...
DOMDocumentHandle CreateXmlTpcSettings() {
    ...
    for (int dev_no = 0; dev_no < num_devices; ++dev_no) {
        ...
        AddAttribute(e_device, L"Url", StringToNstring(GetDeviceUrl(dev_no)));
        ...
        for (unsigned board_addr = 0; board_addr < tpc_maxBoards; ++board_addr) {
            if (((dev_info.installedBoards >> board_addr) & 1) != 0) {
                ...
                for (int p = 0; p < kParamTable.size(); p++) {
                    if (kParamTable[p].timebase) {
                        double v;
                        TPC_ErrorCode err = TPC_GetParameter(dev_no, board_addr, 0, kParamTable[p].par, &v);
                        if (err == tpc_errInvalidParameter) {
                            ...
                            CheckStatus(err);
                            AddAttribute(AddLeaf(e_tb_params, L"Parameter", DoubleToString(v)), L"Name",
                                         kParamTable[p].name);
                        }
                    }
                }
                ...
                for (int input_no = 0; input_no < board_info.numberOfInputs; ++input_no) {
                    ...
                    for (int p = 0; p < kParamTable.size(); p++) {
                        if (!kParamTable[p].timebase) {
                            double v;
                            TPC_ErrorCode err = TPC_GetParameter(dev_no, board_addr, input_no, kParamTable[p].par, &v);
                            if (err == tpc_errInvalidParameter) {
                                // Be tolerant with older servers or smaller hardware
                                err = err;
                            }
                                ...
                                CheckStatus(err);
                                AddAttribute(AddLeaf(e_i_params, L"Parameter", DoubleToString(v)), L"Name",
                                             kParamTable[p].name);
                            }
                        }
                    }
                    ...
                    for (int i = 0; i < assoc_count; ++i) {
                        AddAttribute(AddAttribute(AddEmptyElement(e_assocs, L"Association"), L"Board",
                                                  IntToString(assoc_buf[i].boardAddress)),
