...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: System.cpp 46 2025-03-21 10:42:51Z philipp $
...
Device* System::FindDevice(const char* url) {
    for (auto device : m_devices) {
        string url_converted(url);
        if (url_converted == device->Url()) return device;
    }
    return NULL;
}
...
int System::AddDevice(const char* url, int recvTimeOut, int sendTimeOut) {
    ...
    if (d == NULL) {
        ...
        int ix = static_cast<int>(m_devices.size());
...
TPC_ErrorCode System::GetInputInfo(int deviceIx, int boardAddress, int inputNumber, TPC_InputInfo* inputInfo,
                                   int structSize) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
TPC_ErrorCode System::GetInputRanges(int deviceIx, int boardAddress, int inputNumber, int inputType,
                                     double* inputRanges, int size) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->GetInputRanges(boardAddress, inputNumber, inputType, inputRanges, size);
}
...
TPC_ErrorCode System::ReleaseSystemLock() {
    vector<Device*> devices;
    GetSortedDevicesList(devices);
    for (int index = 0; index < (int)devices.size(); index++) devices[index]->ReleaseDeviceLock();
    return tpc_noError;
}
...
bool System::IsBoardParameter(TPC_Parameter parameter) {
    ...
    switch (static_cast<int>(parameter)) {
        ...
        case tpc_parPllAdcPhase:
            // case tpc_parPllPhase:
            return true;
...
bool System::IsTimebaseParameter(TPC_Parameter parameter) {
    switch (parameter) {
        ...
        case tpc_parMultiplexerMode:  // Deprecated
        ...
        case tpc_parSyncDelay:                   return true;
        default:                                 return false;
    }
}
...
TPC_ErrorCode System::ClusterNumberToBoardAddress(int clusterNumber, int& boardAddress) {
    ...
    for (int index = 0; index < (int)devices.size(); index++) {
        ...
        if (err != tpc_errInvalidBoardAddress) return err;
    }
...
TPC_ErrorCode System::SetTimebaseParameter(int clusterNumber, TPC_Parameter parameter, double value) {
    for (int index = 0; index < (int)m_devices.size(); index++) {
        ...
        if (err != tpc_noError && err != tpc_errInvalidBoardAddress) return err;
    }
    return tpc_noError;
}
...
class GroupParameterRestriction {
   public:
    GroupParameterRestriction() {
        value     = INT_MAX;
        errorCode = tpc_errInvalidBoardAddress;
    }

    TPC_ErrorCode operator()(Device* device, int boardAddress) {
        if (errorCode != tpc_noError && errorCode != tpc_errInvalidBoardAddress) return errorCode;
        TPC_BoardInfo info;
        errorCode = device->GetBoardInfo(boardAddress, &info, sizeof(TPC_BoardInfo));
        if (errorCode != tpc_noError) return errorCode;
...
class OperationModeOptions : public GroupParameterRestriction {
    void CheckRestriction(TPC_BoardInfo& info) { value &= info.operationModeOptions; }
};

class MaximumSampleRate : public GroupParameterRestriction {
    void CheckRestriction(TPC_BoardInfo& info) { value = min(value, info.maxSpeed); }
};

class MaximumBlockLength : public GroupParameterRestriction {
    void CheckRestriction(TPC_BoardInfo& info) { value = min(value, info.maxMemory); }
};

class MaximumNumberOfInputs : public GroupParameterRestriction {
   public:
...
TPC_ErrorCode System::HasCluster(bool* clustered) {
    *clustered = false;
    ...
    if (err != tpc_noError) return err;
    if (m_devices.size() > 1 || clusterNumbers.size() > 1) *clustered = true;
    return tpc_noError;
}
...
TPC_ErrorCode System::RestrictParameterValue(int clusterNumber, TPC_Parameter parameter, double* value) {
    ...
    if (parameter == tpc_parOperationMode) {
        auto op_mode = enum_cast<TPC_OperationModes>(*value);
        if (op_mode == tpc_opModeEventRecorderSingle || op_mode == tpc_opModeEventRecorderSingleDual ||
            op_mode == tpc_opModeEventRecorderMulti || op_mode == tpc_opModeEventRecorderMultiDual) {
            ...
            if (err != 0) return err;
            if ((operationModeOptions.GetValue() & tpc_optEcrExtensions) == 0) *value = tpc_opModeScope;
        }
    }
        ...
        *value      = (double)sampleRate.GetValue() / divisor;
        ...
        // Cap block length to memory size
        int channelMemory = blockLength.GetValue();
        ...
        if (err != 0) return err;
        if (clustered) *value = 1;
    }
    return err;
}

class DeferredParameterSet {
   public:
    DeferredParameterSet(System* system) {
        this->system = system;
        system->BeginSet();
        cancel = true;
    }

    ~DeferredParameterSet() {
        if (cancel) system->CancelSet();
    }
...
TPC_ErrorCode System::SynchronizeTimebaseParameters() {
    DeferredParameterSet parameterSet(this);
...
TPC_ErrorCode System::SetParameter(int deviceIx, int boardAddress, int inputNumber, TPC_Parameter parameter,
                                   double value) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
TPC_ErrorCode System::GetParameter(int deviceIx, int boardAddress, int inputNumber, TPC_Parameter parameter,
                                   double* value) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
TPC_ErrorCode System::SetAttribute(int deviceIx, int boardAddress, int inputNumber, const char* key,
                                   const char* value) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
TPC_ErrorCode System::SetTrigger(int deviceIx, int boardAddress, int inputNumber, TPC_EasyTriggerMode mode,
...
TPC_ErrorCode System::GetTrigger(int deviceIx, int boardAddress, int inputNumber, TPC_EasyTriggerMode* mode,
                                 TPC_EasyTriggerComperatorMode* comp, TPC_EasyTriggerFlags* flags, double* level,
                                 double* hysteresis, int* time, int* time2) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
class SystemLock {
   public:
    ...
    ~SystemLock() {
        if (release) system->ReleaseSystemLock();
    }
...
// TODO Create abstraction for iterating over devices
TPC_ErrorCode System::ExecuteSystemCommand(TPC_SystemCommand command) {
    ...
    if (command == tpc_cmdStart) {
        ...
        if (waitTime > 0) NonWaitableThread::sleep((int)waitTime * 1000);
...
TPC_ErrorCode System::GetYMetaData(int deviceIx, int boardAddress, int inputNumber, int measurementNumber,
...
TPC_ErrorCode System::GetMetaDataParameter(int deviceIx, int boardAddress, int inputNumber, int measurementNumber,
...
TPC_ErrorCode System::GetMetaDataAttribute(int deviceIx, int boardAddress, int inputNumber, int measurementNumber,
...
TPC_ErrorCode System::GetTMetaData(int deviceIx, int boardAddress, int blockNumber, int measurementNumber,
...
TPC_ErrorCode System::GetTMetaData(int deviceIx, int boardAddress, int blockNumberFrom, int blockNumberTo,
                                   int measurementNumber, TPC_TMetaData* metaData, int structSize) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->GetTMetaData(boardAddress, blockNumberFrom, blockNumberTo, measurementNumber, metaData, structSize);
}

TPC_ErrorCode System::GetData(int deviceIx, int boardAddress, int inputNumber, int blockNumber, int measurementNumber,
                              uint64_t dataStart, uint64_t dataLength, int resultLength, double* data,
                              Device::ReadoutType type) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->GetData(boardAddress, inputNumber, blockNumber, measurementNumber, dataStart, dataLength, resultLength,
                      data, type);
}

TPC_ErrorCode System::GetRawData(int deviceIx, int boardAddress, int inputNumber, int blockNumber,
                                 int measurementNumber, uint64_t dataStart, uint64_t dataLength, int resultLength,
                                 int32_t* data, Device::ReadoutType type) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->GetRawData(boardAddress, inputNumber, blockNumber, measurementNumber, dataStart, dataLength, resultLength,
                         data, type);
}

TPC_ErrorCode System::DeferredGetData(int deviceIx, int boardAddress, int inputNumber, int blockNumber,
                                      uint64_t dataStart, uint64_t dataLength, int resultLength, double* data,
                                      TPC_ErrorCode* error, Device::ReadoutType type) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->DeferredGetData(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength, data, error,
                              type);
}
...
TPC_ErrorCode System::DeferredGetRawData(int deviceIx, int boardAddress, int inputNumber, int blockNumber,
    ...
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
    return d->DeferredGetRawData(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength, data,
                                 error, type);
}
...
TPC_ErrorCode System::WriteDev(int deviceIx, int boardAddress, int type, unsigned count, void* data, unsigned aux1,
                               unsigned aux2) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
...
TPC_ErrorCode System::ReadDev(int deviceIx, int boardAddress, int type, unsigned count, void* data, unsigned aux1,
                              unsigned aux2) {
    Device* d = FindDevice(deviceIx);
    if (d == NULL) return tpc_errInvalidDeviceIx;
