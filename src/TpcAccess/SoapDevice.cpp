...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: SoapDevice.cpp 46 2025-03-21 10:42:51Z philipp $
...
void Assign(TPC_BoardInfo& d, const ns__BoardInfo& s) {
    ...
    d.firmwareCapabilities = s.firmwareCapabilities;
}
...
static void* dime_write_open(struct soap* soap, const char* id, const char* type, const char* options) {
    // we can return NULL without setting soap->error if we don't want to use the streaming callback for this DIME
    // attachment
...
static int dime_write(struct soap* soap, void* handle, const char* buf, size_t len) {
    ...
    r->PushBytes((uint8_t*)buf, static_cast<int>(len));
    return SOAP_OK;
}
...
TPC_ErrorCode SoapDevice::GetInterfaceVersion(int& version) {
    ...
    if (err == -1) err = soap_call_ns__GetInterfaceVersion(&m_soap, m_url.c_str(), NULL, version);
...
TPC_ErrorCode SoapDevice::ResetConfiguration() {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__ResetConfiguration(&m_soap, m_url.c_str(), NULL, &Response);
...
TPC_ErrorCode SoapDevice::AcquireDeviceLock() {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__AcquireDeviceLock(&m_soap, m_url.c_str(), NULL, &Response);
...
TPC_ErrorCode SoapDevice::ReleaseDeviceLock() {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__ReleaseDeviceLock(&m_soap, m_url.c_str(), NULL, &Response);
...
TPC_ErrorCode SoapDevice::GetClusterConfiguration(int* clusters) {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__GetClusterConfiguration(&m_soap, m_url.c_str(), NULL, clusterNumbers);

    if (err == 0) {
        for (int i = 0; i < tpc_maxBoards; i++) clusters[i] = clusterNumbers.clusterNumbers[i];
    }
    else {
        for (int i = 0; i < tpc_maxBoards; i++) clusters[i] = -1;
    }
...
TPC_ErrorCode SoapDevice::SetClusterConfiguration(int* clusters) {
    ns__ClusterConfiguration clusterNumbers;
    for (int i = 0; i < tpc_maxBoards; i++) clusterNumbers.clusterNumbers[i] = clusters[i];
    ns__DummyResponse Response;
    int err = soap_call_ns__SetClusterConfiguration(&m_soap, m_url.c_str(), NULL, clusterNumbers, &Response);
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__SetClusterConfiguration(&m_soap, m_url.c_str(), NULL, clusterNumbers, &Response);
...
TPC_ErrorCode SoapDevice::GetHardwareInfo(DeviceInfo& deviceInfo) {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__GetHardwareInfo(&m_soap, m_url.c_str(), NULL, h);
    if (err == 0) {
        if (h.device.serverSoftwareVersion > 10101) {
            ...
            // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
            if (err2 == -1) err2 = soap_call_ns__GetMACAddress(&m_soap, m_url.c_str(), NULL, myIp, macAddress);
            if (err2 == -1) macAddress = 0;
        }
        deviceInfo.Clear();
...
TPC_ErrorCode SoapDevice::SetHardwareSettings(TPC_DeviceInfo deviceInfo) {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__SetDeviceSettings(&m_soap, m_url.c_str(), NULL, settings, &Response);
...
TPC_ErrorCode SoapDevice::GetFreeDiskSpace(uint64_t* freeDiskSpace, uint64_t* DiskSize) {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__GetFreeDiskSpace(&m_soap, m_url.c_str(), NULL, ds);
...
TPC_ErrorCode SoapDevice::GetInputRanges(int boardAddress, int inputNumber, int inputType, double* inputRanges,
                                         int size) {
    ns__InputRangesList list;

    int err = soap_call_ns__GetInputRanges(&m_soap, m_url.c_str(), NULL, boardAddress, inputNumber, inputType, list);
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1)
        err = soap_call_ns__GetInputRanges(&m_soap, m_url.c_str(), NULL, boardAddress, inputNumber, inputType, list);

    if (err == 0) {
        if (size > list.__ptr->NumberOfRanges) size = list.__ptr->NumberOfRanges;
        memcpy(inputRanges, list.__ptr, size * sizeof(double));
    }
    return Epilog();
}
...
TPC_ErrorCode SoapDevice::SetOneParameter(int boardAddress, int inputNumber, int parameterIndex, double value,
                                          double* roundedValue, int* error) {
    ns__SetOneParameterResult r;
    int err = soap_call_ns__SetOneParameter(&m_soap, m_url.c_str(), NULL, boardAddress, inputNumber, parameterIndex,
                                            value, r);
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1)
        err = soap_call_ns__SetOneParameter(&m_soap, m_url.c_str(), NULL, boardAddress, inputNumber, parameterIndex,
                                            value, r);
...
TPC_ErrorCode SoapDevice::GetAllCurrentParameters(ParameterSet& parameters) {
    ...
    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) err = soap_call_ns__GetAllCurrentParameters(&m_soap, m_url.c_str(), NULL, list);

    parameters.ClearParameters();
    if (err == 0) {
        for (int i = 0; i < list.__size; i++) {
            parameters.SetParameter(list.__ptr[i].boardAddress, list.__ptr[i].inputNumber, list.__ptr[i].parameterIndex,
                                    list.__ptr[i].value);
        }
    }
    return Epilog();
...
TPC_ErrorCode SoapDevice::GetAllParametersAvailableValues(ParameterSet& parameters) {
    ns__ParameterAvailableValuesList response;

    int err = soap_call_ns__GetAllParametersAvailableValues(&m_soap, m_url.c_str(), NULL, response);

    // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
    if (err == -1) {
        err = soap_call_ns__GetAllParametersAvailableValues(&m_soap, m_url.c_str(), NULL, response);
    }

    parameters.ClearAvailableValues();
    if (err == 0) {
        for (auto item : response.list) {
            parameters.SetParameterAvailableValues(
                item->boardAddress, item->inputNumber, item->parameterIndex, std::move(item->availableValues),
                static_cast<TPC_ParameterAvailableValuesType>(item->availableValuesType));
        }
    }
    return Epilog();
}
...
TPC_ErrorCode SoapDevice::GetAllCurrentAttributes(Attributes& result) {
    ...
    if (err == 0) {
        for (int i = 0; i < list.__size; i++) {
            result.SetAttribute(list.__ptr[i].boardAddress, list.__ptr[i].inputNumber, list.__ptr[i].name,
                                list.__ptr[i].value);
        }
    }
    return Epilog();
...
TPC_ErrorCode SoapDevice::SetAssociations(const AssociationRowList& associations) {
    ...
    for (int j = 0; j < nRows; j++) {
        ...
        rowD->associatedChannels.__ptr =
            (ns__AssociationChannel*)soap_malloc(&m_soap, sizeof(ns__AssociationChannel) * min);
...
TPC_ErrorCode SoapDevice::GetAllYTMetadata(int currentMeasurementNumber, YMetaDataList& metadata) {
    ...
    int err = soap_call_ns__GetAllYMetadata(&m_soap, m_url.c_str(), NULL, currentMeasurementNumber, list);
    if (err == 0) {
        for (int i = 0; i < list.__size; i++) {
            RawYMetaData m;
...
TPC_ErrorCode SoapDevice::GetMetadata(int currentMeasurementNumber, int boardAddress, int blockFrom, int blockTo,
                                      ns__Metadata metadata[]) {
    ns__MetadataList list;
    int err = soap_call_ns__GetMetadata(&m_soap, m_url.c_str(), NULL, currentMeasurementNumber, boardAddress, blockFrom,
                                        blockTo, list);
...
/// send server custom command (no function)
TPC_ErrorCode SoapDevice::SendServerCustomCommand(char* command, char* result, int length) {
    ...
    if (err == tpc_noError && bufferTooShort) return tpc_errBufferTooShort;
    return err;
}
...
TPC_ErrorCode SoapDevice::CheckSoapError() {
    ...
    if (m_soap.error) {
        ...
        if (!*soap_faultcode(&m_soap)) soap_set_fault(&m_soap);

        const char** s = soap_faultcode(&m_soap);
        if (s && *s) lastFaultCode = *s;

        s = soap_faultstring(&m_soap);
        if (s && *s) lastFaultString = *s;

        s = soap_faultdetail(&m_soap);
        if (s && *s) lastFaultDetail = *s;

        if (m_soap.error == 2 && (lastFaultCode == "SOAP-ENV:Server") &&
