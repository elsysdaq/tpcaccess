// $Id: XercesUtils.cpp 45 2025-03-19 12:46:37Z philipp $
...
unsigned utf16_length(const uint32_t* ucs4) {
    ...
    while (*ucs4) {
        ++size;
        if (*ucs4 >= 0x00010000L && *ucs4 <= 0x0010ffffL) ++size;
...
unsigned utf16_decodechar(const uint16_t* utf16, uint32_t* ucs4) {
    ...
    for (;;) {
        if (utf16[0] < 0xd800 || utf16[0] > 0xdfff) {
            *ucs4 = utf16[0];
            return static_cast<unsigned>(utf16 - start + 1);
        }
        else if (utf16[0] >= 0xd800 && utf16[0] <= 0xdbff && utf16[1] >= 0xdc00 && utf16[1] <= 0xdfff) {
            *ucs4 = (((utf16[0] & 0x03ffL) << 10) | ((utf16[1] & 0x03ffL) << 0)) + 0x00010000L;
            return static_cast<unsigned>(utf16 - start + 2);
        }
...
void utf16_decode(const uint16_t* utf16, uint32_t* ucs4) {
    do utf16 += utf16_decodechar(utf16, ucs4);
    while (*ucs4++);
}

void utf16_encode(uint16_t* utf16, const uint32_t* ucs4) {
    do utf16 += utf16_encodechar(utf16, *ucs4);
    while (*ucs4++);
}
...
static void AppendExceptionMessage(nstring& messages, const X::SAXParseException& e, const TCHAR* type) {
    ...
    message << type << L" at File \"" << TO_WCHAR(e.getSystemId()) << L"\" line " << e.getLineNumber() << L" column "
            << e.getColumnNumber() << L"\n  Message: " << TO_WCHAR(e.getMessage()) << L"\n";
...
DOMDocumentHandle BasicDOMInOut::Parse(const TCHAR* filename, X::InputSource* input) {
    ...
    try {
        ...
        errors     = static_cast<unsigned>(parser.getErrorCount());
