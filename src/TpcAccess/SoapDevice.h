...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: SoapDevice.h 46 2025-03-21 10:42:51Z philipp $
...
class SoapDevice {
   public:
    SoapDevice(const std::string& url, int recvTimeOut, int sendTimeOut);
    ~SoapDevice();
    int KillSocket();
   ...
   public:  // Recording parameters
    TPC_ErrorCode SetOneParameter(int boardAddress, int inputNumber, int parameterIndex, double value,
    ...
    TPC_ErrorCode GetAllParametersAvailableValues(ParameterSet& parameters);
   ...
   public:  // Data readout
    ...
    TPC_ErrorCode GetMetadata(int currentMeasurementNumber, int boardAddress, int blockFrom, int blockTo,
   ...
   public:  // Auto Sequences
    ...
    TPC_ErrorCode invokeSoap(callable soap_func, Ts&&... args) {
        soap_func(&m_soap, m_url.c_str(), NULL, std::forward<Ts>(args)...);
        return Epilog();
    }

    // Invoke Soap function with dummy response argument
    template <typename callable, typename... Ts>
    TPC_ErrorCode invokeSoapDummy(callable soap_func, Ts&&... args) {
        ns__DummyResponse Response;
        invokeSoap(soap_func, std::forward<Ts>(args)..., &Response);
        return Epilog();
    }

    template <typename callable, typename... Ts>
    TPC_ErrorCode invokeSoapRetry(callable soap_func, Ts&&... args) {
        int err = soap_func(&m_soap, m_url.c_str(), NULL, std::forward<Ts>(args)...);
        // After disconnecting and reconnecting, the SOAP End of File error may appear. One retry.
        if (err == -1) {
            err = soap_func(&m_soap, m_url.c_str(), NULL, std::forward<Ts>(args)...);
        }
        return Epilog();
    }
