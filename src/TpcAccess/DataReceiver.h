...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: DataReceiver.h 45 2025-03-19 12:46:37Z philipp $
...
#include <TpcAccess.h>
#include <common/gsoap-gen/soapH.h>
...
class DataRequest {
   public:
    ...
    DataRequest(int boardAddress, int inputNumber, int blockNumber, void* data, TPC_ErrorCode* error, uint32_t mask,
...
class DataRangeRequest : public DataRequest {
   public:
    // Constructor for "raw data requests"
    DataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart, uint64_t dataLength,
                     int resultLength, void* data, TPC_ErrorCode* error);

    // Constructor for "volt requests"
    DataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart, uint64_t dataLength,
                     int resultLength, void* data, TPC_ErrorCode* error, uint32_t mask, double voltFactor,
                     double voltConstant);
...
class DataMinMaxRequest : public DataRequest {
   public:
    // Constructor for "raw data requests"
    DataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart, uint64_t dataLength,
                      int resultLength, void* data, TPC_ErrorCode* error);
...
class DataEnvelopeRequest : public DataRequest {
   public:
    // Constructor for "raw data requests"
    DataEnvelopeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t* indices, int numIndices,
                        void* data, TPC_ErrorCode* error);

    // Constructor for "volt requests"
    DataEnvelopeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t* indices, int numIndices,
                        void* data, TPC_ErrorCode* error, uint32_t mask, double voltFactor, double voltConstant);
...
class DataReceiver {
   public:  // Client side
    ...
    void Clear();
    void AddDataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                             uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error, uint32_t mask,
                             double factor, double constant);

    void AddRawDataRangeRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                uint64_t dataLength, int resultLength, int32_t* data, TPC_ErrorCode* error);

    void AddDataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                              uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error, uint32_t mask,
                              double factor, double constant);

    void AddRawDataMinMaxRequest(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                 uint64_t dataLength, int resultLength, int32_t* data, TPC_ErrorCode* error);
