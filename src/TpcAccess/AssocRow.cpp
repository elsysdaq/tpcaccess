...
//---------------------------------------------------------------------------
/*--------------------------------------------------------------------------------
  $Id: AssocRow.cpp 45 2025-03-19 12:46:37Z philipp $
...
TPC_ErrorCode AssociationRowList::GetAssociatedChannels(int boardAddress, int inputNumber, TPC_AssociatedChannel* list,
                                                        int* count) const {
    ...
    if (ix < 0) {
        *count = 0;
        ...
        if (*count < m_entries[ix].Count())
            ...
            *count = static_cast<int>(m_entries[ix].Count());
...
TPC_ErrorCode AssociationRowList::SetAssociatedChannels(int boardAddress, int inputNumber, TPC_AssociatedChannel* list,
                                                        int count) {
    int ix = FindEntry(boardAddress, inputNumber);
    if (ix < 0) {
        // Add a new entry
...
int AssociationRowList::FindEntry(int boardAddress, int inputNumber) const {
    for (unsigned int i = 0; i < m_entries.size(); i++) {
        if (m_entries[i].GetBoardAddress() == boardAddress && m_entries[i].GetInputNumber() == inputNumber) {
            return i;
        }
    }
    return -1;
