...
TPC_ErrorCode Device::SetClusterNumbers(int* clusterNumbers)
{
  ...
  // TODO: TpcAccess ben�tigt m_currentStatus.clusters.clusterNumbers f�r die 
  // Umsetzung der Boardadressen. Wenn die ClusterNummern nicht zur�ckgelesen 
  // w�rden, k�nnte es passieren, dass TpcAccess die Boardadressen falsch 
  // umsetzt (passiert z.B beim laden vom tps File). Das Problem ist aber, 
  // dass genau zu diesem Zeitpunkt auch der Thread dabei sein k�nnte die 
  // clusterNumbers zur�ckzulesen, welche aber veraltet sind. Dieses Problem 
  // tritt nicht nur bei den clusterNumbers zu sondern auch bei den Parametern 
  // und Attributen was aber nicht so tragisch ist.
  // Also warten wir bis der Thread sicher fertig ist. Besser w�hre es wenn 
  // wir mit dem Thread synchronisieren k�nnten.
...
void Device::SetParamAttr(const ParameterSet& params, const Attributes& attr, const AssociationRowList& assoc)
{
  ...
  // auserhalb gelocked. Dies ist n�tig da ansonsten Parameter Updates die durch einen Callback
