...
#include <common/utils/containers.h>
#include <common/utils/expected.h>
#include <common/utils/tpcmath.h>
#include <common/types.h>

#include "Device.h"

// The design around the lifetimes and synchronization between StatusObserverThread and Device is very inadequate here.
// A refactor should seperate the shared data into a dedicated object where both classes hold a shared_ptr reference.
...
class StatusObserverThread : public NonWaitableThread {
   public:
    ...
    void SetQuitFlag() { m_shouldTerminate = true; }

   private:
    Device* m_device;
    SoapDevice* m_soap;
    // TODO use std::atomic here
...
StatusObserverThread::StatusObserverThread(Device* device, int recvTimeOut, int sendTimeOut) : NonWaitableThread() {
    m_device = device;
    m_device->addRef();
...
TPC_ErrorCode StatusObserverThread::XErr(TPC_ErrorCode err) {
    ...
    if (err == tpc_errCalibrating || err == tpc_errServerClientVersionConflict ||
        err == tpc_errDeviceInitializationFailure || err == tpc_errHardwareFailure || err == tpc_errInternalError)
        return err;
...
TPC_ErrorCode StatusObserverThread::ReloadParams() {
    ...
    cScopedLock m_statusMutexLock(*(m_device->getSyncObject()));
    ...
    // Fetch parameter available values
    err = m_soap->GetAllParametersAvailableValues(params);
    if (m_shouldTerminate) return err;

    err = XErr(err);
    // Older servers do not support Available Values call
    // gSOAP will return tpc_errNoConnection, so we keep going if this error occurs
    if (err != tpc_noError && err != tpc_errNoConnection) {
        m_device->SetDeviceStatusError(err);
        return err;
    }
    m_device->m_availableValuesStaleFlag = false;  // Mark availableValues cache as up-to-date
...
Device::Device(const char* url, int index, System* system, int recvTimeOut, int sendTimeOut) : m_statusMutex(true) {
    ...
    m_availableValuesStaleFlag = true;
...
Device::~Device() {
    // NonWaitableThread deallocates itself after run() completes
...
TPC_ErrorCode Device::GetInputRanges(int boardAddress, int inputNumber, int inputType, double* inputRanges, int size) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
    ...
    return m_soap->GetInputRanges(boardAddress, inputNumber, inputType, inputRanges, size);
}
...
TPC_ErrorCode Device::BoardAddressToGroupNumber(int boardAddress, int& groupNumber) {
    ...
    if (groupNumber == -1) return tpc_errInvalidBoardAddress;
    return tpc_noError;
}
...
TPC_ErrorCode Device::GetAllClusterNumbers(vector<int>& clusterNumbers) {
    ...
    for (int index = 0; index < tpc_maxBoards; index++) {
        ...
        if (number != -1) {
            ...
            if (!contains) clusterNumbers.push_back(number);
        }
    }
    return tpc_noError;
}
    ...
    if (m_deferredSets) {
        ...
        if (err == 0) {
            ...
            if (chgd) {
                ...
                if (parameter == tpc_parInputMode) {
                    // Mark availableValues cache as being stale
                    // FIXME this approach may not scale if we introduce more dynamically changing parameter ranges
                    m_availableValuesStaleFlag = true;
                }
            }
            m_statusMutex.unlock();
...
TPC_ErrorCode Device::GetParameter(int boardAddress, int inputNumber, TPC_Parameter parameter, double* value) {
    ...
    m_statusMutex.unlock();
    if (!ok) return tpc_errInvalidParameter;

    return tpc_noError;
}

TpcExpect<ParameterAvailableValues> Device::GetParameterAvailableValues(int boardAddress, int inputNumber,
                                                                        TPC_Parameter parameter) {
    bool is_implemented =
        utils::find(common::kAvailableValuesParams, parameter) != common::kAvailableValuesParams.end();
    if (!is_implemented) return tl::unexpected(tpc_errNotImplemented);

    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return tl::unexpected(err);

    err = CheckInput(boardAddress, inputNumber);
    if (err != 0) return tl::unexpected(err);

    if (m_availableValuesStaleFlag) {
        // Reload availableValues cache
        ParameterSet params_copy = m_currentParameters;
        err                      = m_soap->GetAllParametersAvailableValues(params_copy);

        if (err == tpc_noError) {
            m_statusMutex.lock();
            m_currentParameters        = std::move(params_copy);
            m_availableValuesStaleFlag = false;
            m_statusMutex.unlock();
        }
    }
    // Get value from local copy
    m_statusMutex.lock();
    auto lookup = m_currentParameters.GetParameterAvailableValues(boardAddress, inputNumber, parameter);
    m_statusMutex.unlock();

    if (!lookup) {
        // Lookup failed means that we are talking to an older server that has not implemented the available parameters
        // call yet We return a default value instead
        return DefaultAvailableValues(boardAddress, parameter);
    }
    return *lookup;
}
...
TPC_ErrorCode Device::GetAssociatedChannels(int boardAddress, int inputNumber, TPC_AssociatedChannel* list,
                                            int* count) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::GetMetaDataAssociatedChannels(int boardAddress, int inputNumber, int measurementNumber,
...
TPC_ErrorCode Device::SetAssociatedChannels(int boardAddress, int inputNumber, TPC_AssociatedChannel* list, int count) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::SetTrigger(int boardAddress, int inputNumber, TPC_EasyTriggerMode mode,
                                 TPC_EasyTriggerComperatorMode comp, TPC_EasyTriggerFlags flags, double level,
                                 double hysteresis, int time, int time2) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
    ...
    if (comp == tpc_etrgCompNegative || comp == tpc_etrgCompPositive || comp == tpc_etrgCompBoth) {
        if (comp == tpc_etrgCompBoth) {
            switch (mode) {
                ...
                case tpc_etrgShortPulse:     return tpc_errInvalidParameterValue; break;
                default:
                    // ok
                    break;
            }
    ...
    andMask = static_cast<uint32_t>(andMaskConv);

    uint32_t tempMask = 0x01000000u << inputNumber;
    for (uint32_t flagMask = 0x08; flagMask != 0x00; flagMask >>= 1) {
    ...
    andMaskConv = static_cast<double>(andMask);
    ...
    switch (mode) {
        ...
        case tpc_etrgWindow: compMode = (compMode == tpc_etrgCompPositive) ? tpc_trgCompWindowIn : tpc_trgCompWindowOut;
        case tpc_etrgSlope:
            trgMode = 0x00;
        ...
        default: return tpc_errInvalidParameterValue; break;
    }
    ...
    SetParameter(boardAddress, inputNumber, tpc_parShowTrgProduct,
                 ((flags & tpc_etrgShowMultipliedSignal) != 0x00) ? 1.0 : 0.0);
...
TPC_ErrorCode Device::GetTrigger(int boardAddress, int inputNumber, TPC_EasyTriggerMode* mode,
                                 TPC_EasyTriggerComperatorMode* comp, TPC_EasyTriggerFlags* flags, double* level,
                                 double* hysteresis, int* time, int* time2) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
        ...
        switch (tempMode) {
            case tpc_trgBasic:
                switch (tempComp) {
                    ...
                    case tpc_trgCompBothSlopes:    tempMode = tpc_etrgSlope; break;
            ...
            default: return tpc_errNotImplemented; break;
        }
    ...
    if (err != 0) return err;
    if (tempPar != 0) {
        uint32_t andMask  = (static_cast<uint32_t>(tempPar)) & (0x01010101u << inputNumber);
        uint32_t tempMask = 0xff000000;
        for (uint32_t flagMask = 0x08; flagMask != 0x00; flagMask >>= 1) {
    ...
    *mode  = static_cast<TPC_EasyTriggerMode>(tempMode);
    *flags = static_cast<TPC_EasyTriggerFlags>(tempFlags);
    *comp  = static_cast<TPC_EasyTriggerComperatorMode>(tempComp);
...
TPC_ErrorCode Device::SetI2CTrigger(int boardAddress, TPC_I2CTriggerSettings* settings) {
    ...
    if ((settings->mode == tpc_i2cAddressAndData) && (settings->dataLength < 1 || settings->dataLength > 16))
        return tpc_errInvalidParameterValue;
    if ((settings->mode == tpc_i2cAddressAndData) && (settings->dataStartByte < 0 || settings->dataStartByte > 253))
        return tpc_errInvalidParameterValue;
...
TPC_ErrorCode Device::SetCANTrigger(int boardAddress, TPC_CANTriggerSettings* settings) {
    ...
    if ((settings->mode == tpc_canIdAndData) && (settings->dataLength < 0 || settings->dataLength > 8))
        return tpc_errInvalidParameterValue;
...
void Device::FillDeviceStatus(TPC_DeviceStatus* status) {
    ...
    for (int b = 0; b < tpc_maxBoards; b++) {
        ...
        if (ok)
            status->operationModes[b] = (int)value;
        else
            status->operationModes[b] = 99;
    }
...
TPC_ErrorCode Device::GetYMetaData(int boardAddress, int inputNumber, int measurementNumber, TPC_YMetaData* metaData,
                                   int structSize) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::GetMetaDataAttribute(int boardAddress, int inputNumber, int measurementNumber, const char* key,
                                           string& value) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::GetAllMetaDataAttributes(int measurementNumber, TPC_AttributeEnumeratorCallback callback,
                                               void* userData) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::GetTMetaData(int boardAddress, int blockNumber, int measurementNumber, TPC_TMetaData* metaData,
                                   int structSize) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
...
TPC_ErrorCode Device::GetTMetaData(int boardAddress, int blockNumberFrom, int blockNumberTo, int measurementNumber,
    ...
    if (TempMetaData.timeBaseSource == tpc_tbSrcExternal) {
        found = m_previousParameters.GetParameter(boardAddress, 0, tpc_parExtClockSamplingFrequency,
                                                  &TempMetaData.sampleRate);
    }
    else {
    ...
    for (int i = 0; i < iNumberOfBlocks; i++) {
        ...
        metaData[i].singleChannel        = (NewMetaData[i].boardAddress >= 0 && NewMetaData[i].inputNumber >= 0);
...
TPC_ErrorCode Device::GetData(int boardAddress, int inputNumber, int blockNumber, int measurementNumber,
                              uint64_t dataStart, uint64_t dataLength, int resultLength, double* data,
                              ReadoutType type) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
    ...
    if (type == readoutRange)
        dataReceiver.AddDataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength,
                                         data, &error, metaData.analogMask, metaData.binToVoltFactor,
                                         metaData.binToVoltConstant);
    else
        dataReceiver.AddDataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength,
                                          data, &error, metaData.analogMask, metaData.binToVoltFactor,
                                          metaData.binToVoltConstant);
...
TPC_ErrorCode Device::GetRawData(int boardAddress, int inputNumber, int blockNumber, int measurementNumber,
                                 uint64_t dataStart, uint64_t dataLength, int resultLength, int32_t* data,
                                 ReadoutType type) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
    ...
    TPC_ErrorCode error;
    if (type == readoutRange)
        dataReceiver.AddRawDataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength,
                                            data, &error);
    else
        dataReceiver.AddRawDataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength,
...
TPC_ErrorCode Device::DeferredGetData(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                      uint64_t dataLength, int resultLength, double* data, TPC_ErrorCode* error,
                                      ReadoutType type) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;
    ...
    if (type == readoutRange)
        m_dataReceiver.AddDataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength,
                                           data, error, m.analogMask, m.binToVoltFactor, m.binToVoltConstant);
    else
        m_dataReceiver.AddDataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength, resultLength,
                                            data, error, m.analogMask, m.binToVoltFactor, m.binToVoltConstant);

    return tpc_noError;
}

TPC_ErrorCode Device::DeferredGetRawData(int boardAddress, int inputNumber, int blockNumber, uint64_t dataStart,
                                         uint64_t dataLength, int resultLength, int32_t* data, TPC_ErrorCode* error,
                                         ReadoutType type) {
    TPC_ErrorCode err = CheckStatus();
    if (err != 0) return err;

    if (type == readoutRange)
        m_dataReceiver.AddRawDataRangeRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength,
                                              resultLength, data, error);
    else
        m_dataReceiver.AddRawDataMinMaxRequest(boardAddress, inputNumber, blockNumber, dataStart, dataLength,
                                               resultLength, data, error);
...
// Device class lifetime management
// Somewhat ugly and unidiomatic C++ due to retrofitting a manual reference count to something that should be managed
// with shared_ptr
...
[[nodiscard]] bool Device::dropRef() {
    int refs = m_refcount--;
    if (refs < 0) {
        throw std::runtime_error("Negative refcount");
    }
    else if (refs == 0) {
        return true;
    }
...
TPC_ErrorCode Device::FetchCurrentParameters(SoapDevice* soap) {
    ...
    if (changed) {
        // If params changed, we also fetch the new available values arrays
        err = soap->GetAllParametersAvailableValues(p);
        if (err == 0) {
            m_statusMutex.lock();
            m_currentParameters        = p;
            m_availableValuesStaleFlag = false;
...
void Device::SetParamAttr(const ParameterSet& params, const Attributes& attr, const AssociationRowList& assoc) {
    ...
    // ausgelesen werden inkonsistent sind.
    // m_statusMutex.lock();
...
void Device::SetPreviousParamAttr(const ParameterSet& params, const Attributes& attr, const YMetaDataList& metadata,
                                  const AssociationRowList& assoc) {
    m_statusMutex.lock();
...
void Device::SetStatus(const ns__Status& status) {
    ...
    if (m_deviceStatusError != 0) {
        ...
        for (int i = 0; i < tpc_maxBoards; i++) {
            if (m_currentStatus.clusters.clusterNumbers[i] != status.clusters.clusterNumbers[i]) settingsChgd = true;
        }

        // Compare measurement number
        if (m_currentStatus.status.measurementNumber != status.status.measurementNumber) newMeas = true;
    }
...
TpcExpect<ParameterAvailableValues> Device::DefaultAvailableValues(int boardAddress, TPC_Parameter parameter) {
    switch (parameter) {
        case tpc_parScopeBlockLength:
        case tpc_parBlkBlockLength:   {
            int mem        = m_deviceInfo.GetBoardInfo(boardAddress).maxMemory;
            int num_inputs = m_deviceInfo.GetBoardInfo(boardAddress).numberOfInputs;
            // Technically, this is not exactly correct because we should calculate the minimum from all boards in the
            // cluster
            double default_max_blk_length = static_cast<double>(mem / num_inputs);
            ParameterAvailableValues res(utils::pow2array<double>(1024.0, default_max_blk_length), tpc_valsDiscrete);
            return res;
        }
        default: return tl::unexpected(tpc_errNotImplemented);
    }
}
