/*--------------------------------------------------------------------------------
  $Id: TpcAccessElsys.h 918 2026-01-06 15:52:32Z philipp $
...
/// Note: Reading/Writing from/to FPGA registers is not supported on TPCE devices
/** The parameter 'type' takes values from enum eDevIndex.
 *  Conversion hwconst union -> parameters
 *
 * <table>
 *	<tr><th>Type</th>			<th>count</th>		<th>data</th> <th>aux1 </th>	<th> aux2 </th>
 * </tr> <tr><td>sMemoryData</td>	<td>Count</td>		<td>Data</td> <td>Address</td>	<td>Bank</td>	</tr>
 *  <tr><td>sTriggerData</td>	<td>Count</td>		<td>Data</td> <td>0</td>		<td>0</td>
 *	</tr> <tr><td>sEepromData</td>	<td>Count</td>		<td>Data</td> <td>0</td>		<td>0</td>
 *	</tr> <tr><td>sCalVoltageData</td><td>NumVals*4</td>  <td>Data</td> <td>0</td>		<td>0</td>
 *	</tr> <tr><td>sRegisterData</td>	<td>4</td>			<td>&Data</td><td>Dest</td>
 *	<td>Address</td></tr>
 * </table>
 *
 * \param deviceIx Device Index
 ...
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_WriteDev(int deviceIx, int boardAddress, int type, unsigned count, void* data,
 ...
 * \return \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_ReadDev(int deviceIx, int boardAddress, int type, unsigned count, void* data,
                                              unsigned aux1, unsigned aux2);
 ...
 * \return \ref tpc_noError if successful, otherwise an error code indicating the problem.
 */
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_ReadWriteTwi(int deviceIx, int boardAddress, int amplifier, int count,
                                                   char* data);
...
enum TPC_Elsys_Parameter {
    ...
    /// PLL ADC Phase
    tpc_parPllAdcPhase = 3009,
    /// PLL Phase
    tpc_parPllPhase = 3010,
};
...
struct TPC_BitReadoutTestResult {
    int channelBitErrors[tpc_maxInputs];
    int size;
};

/// Trigger ADC test mode and return nummer of bit errors per channel
...
/// Write currently active phase parameters to EEPROM
TPC_EXP enum TPC_ErrorCode TPC_CC TPC_WritePhaseParameters(int deviceIx, int boardAddress);

struct TPC_ParamAvailableValsResult {
    int boardAddress;
    int inputNumber;
    int parameterIndex;
    double availableValues[32];
    int size;
    int type;
};

// TPC_EXP enum TPC_ErrorCode TPC_CC TPC_GetBitReadoutErrors(int deviceIx, int boardAddress, struct
// TPC_BitReadoutTestResult *result);
