/*--------------------------------------------------------------------------------
  $Id: sysThreading.cpp 1000 2026-03-20 09:45:51Z philipp $
...
class SysThreadingException : public std::exception {
   public:
    SysThreadingException(const string& msg) { m_message = msg; }
    virtual ~SysThreadingException() noexcept {}
...
string getSysError() {
    ...
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
                   GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPSTR)&MsgBuf, 0, NULL);
...
void cMutex::unlock() {
    ...
    if (m_lockCount == 0) throw SysThreadingException("Mutex is not locked.");
...
cMutex::~cMutex() {
    ...
    if (Status == EBUSY) {
        // Can't throw inside a destructor
        assert("Destructor called on a locked mutex");
        std::terminate();
    }
}
...
void cMutex::lock() {
    ...
    if (Status == EDEADLK) throw SysThreadingException("Deadlock detected [" + getSysError() + "]");
    assert(Status == 0);
}
...
void NonWaitableThread::start() {
    m_threadHandle = reinterpret_cast<HANDLE>(
        _beginthreadex(0, 0, (unsigned int(__stdcall*)(void*))StartNonWaitableThread, this, 0, &m_threadId));
    if (m_threadHandle == 0) throw SysThreadingException("Could not get a thread handle [" + getSysError() + "]");
}
...
WaitableThread::WaitableThread() : NonWaitableThread() {
    m_running = false;
    m_stopped = true;
}
...
void WaitableThread::wait() {
    ...
    if (m_threadHandle == NULL || m_threadHandle == INVALID_HANDLE_VALUE) {
        // Handle already invalid likely because wait() was called in destructor of threaded object before thread was
        // started
        return;
    }
    int err = WaitForSingleObject(m_threadHandle, INFINITE);
    if (err != 0) throw SysThreadingException("WaitForSingleObject returned an error [" + getSysError() + "]");
...
void WaitableThread::start() {
    m_stopped      = false;
    m_threadHandle = reinterpret_cast<HANDLE>(
        _beginthreadex(0, 0, (unsigned int(__stdcall*)(void*))StartWaitableThread, this, 0, &m_threadId));
    if (m_threadHandle == 0) throw SysThreadingException("Could not get a thread handle [" + getSysError() + "]");
}
...
void NonWaitableThread::start() {
    ...
    if (err != 0) throw SysThreadingException("pthread_create returned an error [" + getSysError() + "]");
}
...
void WaitableThread::wait() {
    ...
    if (err != 0) throw SysThreadingException("pthread_join failed [" + getSysError() + "]");
}
...
void cCondition::broadcast() {
    ...
    for (unsigned int i = 0; i < m_array.size(); i++) SetEvent(m_array[i]);
...
bool cCondition::timedwait(cMutex& externalMutex, unsigned long milliSeconds) {
    ...
    if (event == 0) {
        ...
        if (event == 0) throw SysThreadingException("Could not create event handle [" + getSysError() + "]");
...
cCondition::~cCondition() {
    ...
    if (status == EBUSY) {
        // Can't throw inside a destructor
        assert("The condition variable condition is referenced by another thread");
        std::terminate();
    }
}
...
void cCondition::wait(cMutex& externalMutex) {
    ...
    if (err != 0) throw SysThreadingException("pthread_cond_wait returned an error [" + getSysError() + "]");
}
...
void cCondition::signal() {
    ...
    if (Status != 0) throw SysThreadingException("pthread_cond_signal returned an error [" + getSysError() + "]");
}

void cCondition::broadcast() {
    int Status = pthread_cond_broadcast(&m_condition);
    if (Status != 0) throw SysThreadingException("pthread_cond_broadcast returned an error [" + getSysError() + "]");
}
